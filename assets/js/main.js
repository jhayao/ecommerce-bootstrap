...
addCartBtns.forEach((item) => {
  item.addEventListener("click", () => {
    ...
    const quantity = document.querySelector('#productQuantity').textContent.trim();
    ...
    if (!token) {
                     ...
                     <div class="infor flex items-center gap-3 w-full">
                        ...
                        <div class='w-full'>
                            <div class="flex items-center justify-between w-full">
                                <div class="name text-button">${item.name}</div>
                                <div class="remove-cart-btn remove-btn caption1 font-semibold text-red underline cursor-pointer" data-item=${item.id}>
                                    Remove
                                </div>
                            </div>
        ...
        prdItems.forEach((prd) => {
            ...
            removeCartBtn.addEventListener("click", () => {
                ...
                const newArray = cartStore.filter((item) => item.product_id !== prdId);
                ...
                prdItem.innerHTML = `
                    <div class="infor flex items-center gap-3 w-full">
                        ...
                        <div class='w-full'>
                            ...
                            <div class="flex items-center justify-between gap-2 mt-3 w-full">
                                <div class="flex items-center text-secondary2 capitalize">
                                    <div class="product-price text-title">qty: ${item.pivot.quantity}</div>
                                </div>
                                <div class="product-price text-title">$${item.price * item.pivot.quantity}</div>
                            </div>
                        </div>
                    </div>
                `;
  ...
  if (indicator) {function renderProfile(user) {
    ...
    $.ajax({
        ...
        if (user.customer.first_name && user.customer.last_name) {
            ...
            $('#profileEmail').html(`${user.customer.email}`);
    ...
    `
      ...
      ? `
                ...
                <div
                    ...
                    -${Math.floor(
        100 - (productPrice / product.originPrice) * 100
      )}%
                </div>
        `
      : ""
...
export function addEventToProductItem(products) {
  // Product item
...
function addCartItem(productId, quantity) {
    ...
    if (!token) {
        console.warn('User is not authenticated. Adding item to local storage.');
        let cartStore = localStorage.getItem("cartStore");
        cartStore = cartStore ? JSON.parse(cartStore) : [];
...
export function removedProductFromCart(product_id) {
...
function fetchCartCount() {
    ...
    if (!token) {
        console.warn('User is not authenticated. Fetching cart count from local storage.');
        let cartStore = localStorage.getItem("cartStore");
        cartStore = cartStore ? JSON.parse(cartStore) : [];
        document.querySelector('.cart-quantity').textContent = cartStore.length;
        return;
    }
